                ...
                <form action="mark" method="post">
                    Semester: <select name="semester">
                        <c:forEach items="${listSemester}" var="item">
                            <option value="${item.getSemester()}">${item.getSemester()}</option>
                        </c:forEach>
                    </select>
                    <input type="submit" name="choose" value="Choose"><br>
                    Mark Report Detail:<br><br>
                    <table border="1" style="width: 100%">
                        ...
                        <c:forEach items="${listMarkReport}" var="item">
                            <tr>
                                
                                <td>${item.getClassName()}</td>
                                <td>${item.getSbID()}</td>
                                <td>${item.getSbName()}</td>
                                <td>${item.getInstructorID()}</td>
                                <td>${item.getSemester()}</td>
                                <td>${item.getTotalMark()}</td>
